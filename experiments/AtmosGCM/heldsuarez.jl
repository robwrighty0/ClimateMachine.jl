...
"""
struct HeldSuarezForcing{PV <: Union{Momentum,Energy}} <: TendencyDef{Source, PV} end

HeldSuarezForcing() = (HeldSuarezForcing{Momentum}(),HeldSuarezForcing{Energy}(),)
...
function source(s::HeldSuarezForcing{Energy}, m, state, aux, t, ts, direction, diffusive)
...
function source(s::HeldSuarezForcing{Mass}, m, state, aux, t, ts, direction, diffusive)
    nt = held_suarez_forcing_coefficients(m, state, aux, t, ts, direction, diffusive)
