...
function source(s::BomexGeostrophic{Momentum}, m, state, aux, t, ts, direction, diffusive)
...
BomexSponge(::Type{FT}, args...) where {FT} =
    BomexSponge{Momentum, FT}(args...)

function source(s::BomexSponge{Momentum}, m, state, aux, t, ts, direction, diffusive)
...
"""
struct BomexTendencies{PV<:Union{Mass,Energy,TotalMoisture},FT}  <: TendencyDef{Source, PV}
...
function compute_bomex_tend_params(s, m, state, aux, t, ts, direction, diffusive)
    FT = eltype(state)
    ρ = state.ρ
    z = altitude(m, aux)
...
function source(s::BomexTendencies{Mass}, m, state, aux, t, ts, direction, diffusive)
    params = compute_bomex_tend_params(s, m, state, aux, t, ts, direction, diffusive)
    @unpack ρ∂qt∂t, w_s, k̂ = params
    ρ = state.ρ
    return ρ∂qt∂t - state.ρ * w_s * dot(k̂, diffusive.moisture.∇q_tot)
end
function source(s::BomexTendencies{Energy}, m, state, aux, t, ts, direction, diffusive)
    params = compute_bomex_tend_params(s, m, state, aux, t, ts, direction, diffusive)
    FT = eltype(state)
...
function source(s::BomexTendencies{TotalMoisture}, m, state, aux, t, ts, direction, diffusive)
    params = compute_bomex_tend_params(s, m, state, aux, t, ts, direction, diffusive)
    @unpack ρ∂qt∂t, w_s, k̂ = params
    return ρ∂qt∂t - state.ρ * w_s * dot(k̂, diffusive.moisture.∇q_tot)
end

filter_source(pv::PV, s::BomexTendencies{PV}) where {PV<:PrognosticVariable} = s
filter_source(pv::PV, s::BomexSponge{PV}) where {PV<:PrognosticVariable} = s
filter_source(pv::PV, s::BomexGeostrophic{PV}) where {PV<:PrognosticVariable} = s
