...
#JK if !@isdefined integration_testing
#JK     const integration_testing = parse(
#JK         Bool,
#JK         lowercase(get(ENV, "JULIA_CLIMA_INTEGRATION_TESTING", "false")),
#JK     )
#JK end
#JK const output = parse(Bool, lowercase(get(ENV, "JULIA_CLIMA_OUTPUT", "false")))
...
struct Pseudo1D{n, α, μ, δ} <: AdvectionDiffusionProblem end
...
function test_run(
    ...
    model = AdvectionDiffusion{dim}(Pseudo1D{n, α, μ, δ}(), diffusion = false)
    dg = DGModel(
        model,
        grid,
        RusanovNumericalFlux(),
        ...
        for FT in (Float64, Float32)
            ...
            for dim in 2:3
                ...
                for l in 1:numlevels
                    ...
                    #JK @test result[l] ≈ FT(expected_result[dim, l, FT])
                    if l > 1
